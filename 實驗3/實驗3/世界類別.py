...
class 地圖類別:
    ...
    def 顯示(self):
        ...
        if self.世界.腐化物分解者死亡數 != 0:
            腐化物分解者死亡時平均生命回合數 = self.世界.腐化物分解者死亡時總生命刻數 / 10 / self.世界.腐化物分解者死亡數
        print("腐化物分解者死亡時平均生命回合數 = ", round(腐化物分解者死亡時平均生命回合數, 2), " " * 20)
        print(顯示地圖字串)
        print(
...
class 世界類別(可儲存物件類別):
    ...
    def 更新世界(self):
        ...
        if self.刻數 % 10 == 0:
            ...
            self.更新地圖顯示()
            # 開始下一回合的改變
            if self.回合數 >= 20:  # 20 回合後(生態環境較穩定)再生成腐化物分解者
                self.隨機生成腐化物分解者()

        self.腐化物分解者移動()

        if self.刻數 % 10 == 0:
            self.氣味場.更新氣味傳播()  # 根據現有氣味產生下一回合氣味的傳播
            self.產生氣味場()  # 現有東西的位置產生氣味
            self.草地生成植物()
            self.空地生成草地()
            self.處理植物死亡()
            self.處理腐化物死亡()
