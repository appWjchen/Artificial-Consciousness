...
class 腐化植物分解者類別(生態環境物品類別):
    ...
    def 處理死亡(self):
        ...
        if self.能量 <= 0:  # 能量用完就算死了, 記錄移動總數, 重設能量
            ...
            self.世界.腐化植物分解者死亡時總生命回合數 += self.生命回合數
            self.能量 = 腐化植物分解者類別.預設能量
            self.生命回合數 = 0
            self.生命代數 += 1
    ...
    def 移動(self):
        ...
        if (
            ...
            if self.世界.地上格子[new_x][new_y] == -1:
                ...
                self.x = new_x
                self.y = new_y
                self.能量 -= 腐化植物分解者類別.預設移動消秏能量
        elif (
            self.世界.地面格子[new_x][new_y].形狀 == 生態環境物品類別.腐化植物形狀
            and self.世界.地上格子[new_x][new_y] == -1
        ):  # 前往的格子是腐化植物 且 上面沒有其它生物, 注意 if 和 elif 順序不能交換, 要先確定格子不是空地, 否則會出錯(空地沒有形狀)
